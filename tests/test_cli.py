...
class TestMainDirect:
    ...
    def test_theme_light(self, tmp_path, monkeypatch):
        ...
        assert main(['--theme=light', '--export', str(tmp_path),
                      out, '--quiet']) == 0
    ...
    def test_theme_dark(self, tmp_path, monkeypatch):
        ...
        md.write_text('# Hi')
        out = str(tmp_path / 'out.html')
        assert main(['--theme=dark', '--export', str(tmp_path),
                      out, '--quiet']) == 0
    ...
    def test_export_with_title(self, tmp_path, monkeypatch):
        ...
        out = str(tmp_path / 'out.html')
        assert main(['--export', '--title=Custom', str(tmp_path),
                      out, '--quiet']) == 0
        with open(out) as f:
    ...
    def test_export_no_inline(self, tmp_path, monkeypatch):
        ...
        out = str(tmp_path / 'out.html')
        assert main(['--export', '--no-inline', str(tmp_path),
                      out, '--quiet']) == 0
        with open(out) as f:
            html = f.read()
